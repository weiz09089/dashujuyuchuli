...
let bookmarkedQuestions = JSON.parse(localStorage.getItem('bd_bookmarks')) || [];
let userProgress = JSON.parse(localStorage.getItem('bd_progress')) || {
...
function renderQuestion() {
    ...
    const typeNames = { 'judge': '判断题', 'choice': '选择题' };
...
function selectJudge(userAnswer) {
    ...
    showAnswerResult(correct, q.answer ? '正确(A)' : '错误(B)');
}
...
function saveProgress() {
    localStorage.setItem('bd_progress', JSON.stringify(userProgress));
    localStorage.setItem('bd_bookmarks', JSON.stringify(bookmarkedQuestions));
}

function resetAll() {
    if (confirm('确定清除所有进度和收藏吗？')) {
        localStorage.removeItem('bd_progress');
        localStorage.removeItem('bd_bookmarks');
        location.reload();
    }
}
